...
with open("data/phone_links.txt", "r") as f:
    for line in f:
        links.append(line[:-1])
        # if re.search('oneplus',line[:-1]):
        #     links.append(line[:-1])
filepath = '../data/gsmarena/specs.csv'
with open("data/ip_addresses.txt", "r") as f:
    for i in range(3):
        ip_addresses.append(f.readline()[0:-1])
headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
    'Referer': 'https://example.com/'  # Replace with the appropriate referer
}
...
def get_specs(html,index):
    ...
    # popularity1 = None # preventing error custom 
...
for index,link in enumerate(links):
    ...
    try:
        ...
        #     break
        spec  = get_specs(response,index)
        print(f"{index} links are done")
        ...
        # specs= {'Name':name,  'Released':released,
        #         'Dimension':dimension, 'Weight':weight, 'Build':build, 'Sim':sim, 'Body(Other)':bodyother,
        #         'Display Type':displaytype, 'Display Size':displaysize, 'Resolution':displayresolution, 'Display Protection':displayprotection,
        #         'Display(Other)':displayother, 'OS':os, 'Chipset':chipset, 'CPU':cpu, 'GPU':gpu, 
        #         'Memory(Internal)':internalmemory, 'Main Camera':cam1modules, 'Main Camera(Features)':cam1features,
        #         'Main Camera(Video)':cam1video, 'Front Camera':cam2modules, 'Front Camera(Features)':cam2features,
        #         'Front Camera(Video)':cam2video, 'Speker':loudspeaker, 'Headphone Jack':jack, 
        #         'USB':usb,'Sensors':sensors,'Battery Type':bat_type, 'Charging':charging,
        #         'Battery Talktime':battalktime1, 'Colors':colors, 'Models':model_names, 'SAR':sar_us,
        #         'SAR EU':sar_eu, 'Price':price, 'Benchmark Results':benchmarks,
